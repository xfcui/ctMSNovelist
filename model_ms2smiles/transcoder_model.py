...
class TranscoderModel(BlueprintedModel):
    ...
    def forward(self, inputs):
        ...
        z, rnn_states_ = self.encoder(fingerprints_, formula)
        rnn_states = [rnn_states_.detach(), torch.zeros_like(rnn_states_).detach().to(rnn_states_.device)]
        ...
        # (batch_size, 127, 306)
        decoder_input = torch.cat([inputs['tokens_X'],  # (batch_size, seq_len, 39)
                         element_grammar_count,  # (batch_size, seq_len, 11)
                         z_repeated], dim=-1)    # (batch_size, seq_len, 256)
